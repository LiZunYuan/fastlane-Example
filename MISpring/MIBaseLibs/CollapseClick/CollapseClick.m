...
-(void)reloadCollapseClick {
    ...
    for (int xx = 0; xx < [CollapseClickDelegate numberOfCellsForCollapseClick]; xx++) {
        // Create Cell
        CollapseClickCell *cell = [CollapseClickCell newCollapseClickCellWithTitle:[CollapseClickDelegate titleForCollapseClickAtIndex:xx] width:self.frame.size.width index:xx content:[CollapseClickDelegate viewForCollapseClickContentViewAtIndex:xx]];
        ...
        // Set cell.Border's Color
        if ([(id)CollapseClickDelegate respondsToSelector:@selector(colorForViewBorder:)]) {
            [cell.layer setBorderColor: [CollapseClickDelegate colorForViewBorder:xx].CGColor];
        ...
        totalHeight += kCCHeaderHeight + kCCPad / 2;
    }
