...
@synthesize deleting;
@synthesize deleter, deletable, onDelete;
...
- (void) enableDeleting {
    ...
    animation.toValue=[NSNumber numberWithFloat:0.85];
    animation.duration=0.2;
    animation.autoreverses=NO;
    animation.removedOnCompletion=NO;
    animation.fillMode=kCAFillModeForwards;
    
    [[self layer] addAnimation:animation forKey:@"transtionScaleKey"];
}
...
- (void)setOnDelete:(Block)_onDelete {
    onDelete = [_onDelete copy];
    if (onDelete) {
        self.deletable = YES;
    }
}
...
- (UITapGestureRecognizer *)deleter {
    if (!deleter) {
        deleter = [[UITapGestureRecognizer alloc]
                   initWithTarget:self action:@selector(deleted)];
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)recogniser
    ...
    if (self.deleting && touch.view.tag != MIAPP_FAVS_DELETE_IMAGEVIEW_TAG) {
        return NO;
    }
