...
- (void)tapped {
    if (self.onTap && self.deleting) {
        self.onTap();
    }
}

- (void)longPressed {
    if (self.onLongPress && !self.deleting) {
        self.onLongPress();
    }
}
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)recogniser
    ...
    if ((recogniser == self.panGestureRecognizer || recogniser
         == self.gestureRecognizers[0]) && (touch.view.tag != MIAPP_FAVS_DELETE_IMAGEVIEW_TAG) && (recogniser.state == UIGestureRecognizerStateBegan)) {
        return YES;
    } else {
        return ![touch.view isKindOfClass:UIControl.class] && (touch.view.tag != MIAPP_FAVS_DELETE_IMAGEVIEW_TAG);
    }    
}
...
- (UILongPressGestureRecognizer *)longPresser {
    if (!longPresser) {
        ...
        [tapper requireGestureRecognizerToFail:longPresser];
    }
    return longPresser;
}
#pragma mark - Fini
