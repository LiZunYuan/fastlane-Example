...
NSString *completeRPCURL = WEBVIEW_COMPLETE_URL;
...
{
    BOOL ret = YES;
    if (_webViewProxyDelegate && [_webViewProxyDelegate respondsToSelector:@selector(webView:shouldStartLoadWithRequest:navigationType:)]) {
...
{
    if (_webViewProxyDelegate && [_webViewProxyDelegate respondsToSelector:@selector(webViewDidStartLoad:)]) {
...
{
    if (_webViewProxyDelegate && [_webViewProxyDelegate respondsToSelector:@selector(webViewDidFinishLoad:)]) {
...
{
    if (_webViewProxyDelegate && [_webViewProxyDelegate respondsToSelector:@selector(webView:didFailLoadWithError:)]) {
...
- (NSMethodSignature *)methodSignatureForSelector:(SEL)selector
{
    ...
    if(!signature) {
        if(_webViewProxyDelegate && [_webViewProxyDelegate respondsToSelector:selector]) {
...
{
    if (_webViewProxyDelegate && [_webViewProxyDelegate respondsToSelector:[invocation selector]]) {
